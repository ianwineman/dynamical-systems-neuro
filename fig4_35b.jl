...
# ╔═╡ bc34703e-88ad-11ef-1e30-158846a4b6bb
using DifferentialEquations, Plots

# ╔═╡ 264be335-4b4f-4987-986c-307bd6d60c73
md"""
# Figure 4.35b
"""

# ╔═╡ ac6aa36a-6c90-46ea-8424-ac7139c32902
function m∞(V)
	Vhalf, k = -20, 15
	return 1 / (1 + exp((Vhalf - V) / k))
end

# ╔═╡ acab3b84-f962-4779-a5be-c66d4fb570ab
function n∞(V)
	Vhalf, k = -45, 5
	return 1 / (1 + exp((Vhalf - V) / k))
end

# ╔═╡ 157c527e-702e-415f-ba85-fb2aec0fccf1
function INapIK_model!(du, u, p, t)
	V, n = u 
	gL, EL, gNa, ENa, gK, EK = [8, -78, 20, 60, 10, -90]
	I = p

	# ̇V = I - gL(V-EL) - gNa * m∞(V)(V-ENa) - gK * n(V-EK)
    du[1] = I(t) - gL * (V - EL) - gNa * m∞(V) * (V - ENa) - gK * n * (V - EK)

	# ̇n = n∞(V) - n
	du[2] = (n∞(V) - n)
end

# ╔═╡ f0c69d66-71cc-4646-8f9b-bce04c1f3434
begin
	min_V, max_V = [], []
	...
	for I=-20:0.1:100
		...
		model = ODEProblem(INapIK_model!, u0, timespan, t -> I)
		solution = solve(model)

		transient_period_end = floor(Int, length(solution.t)/3)
		push!(min_V, minimum(solution[1, transient_period_end:end]))
		push!(max_V, maximum(solution[1, transient_period_end:end]))
	end
end

# ╔═╡ 47366504-cde9-4db8-8c5c-83614aab54df
begin
	plot(
		[i for i=-20:0.1:100],
		[max_V, min_V],
		label=["max V(t)" "min V(t)"],
		color=[:red :blue],
		xlim=[-20,100],
		ylim=[-80,20],
		linewidth=2,
		title="Figure 4.35: Hopf bifurcation diagram in the \$I_{Na,p}+I_K\$-model",
		titlefontsize=10,
		xguide="injected dc-current, I",
		xguidefontsize=8,
		yguide="membrane voltage, V",
		yguidefontsize=8
	)

	# annotation locations approximate
	annotate!(0, -20, ("supercritical\nAndronov-Hopf\nbifurcation", :left, 8))
	quiver!([8], [-28], quiver=([0], [-27]), color=:black)
end

# ╔═╡ b6720b2e-ff97-4b5c-ac70-d0ef400592e7
savefig("fig4_35b")

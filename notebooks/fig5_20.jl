...
# ╔═╡ 567eaeb4-8d85-11ef-0df9-3b19dbfb0077
using Plots, DifferentialEquations, Measures

# ╔═╡ 08216bae-8150-4e71-b5f6-8dccade2d203
...
# ╔═╡ 2930e906-ef37-44e8-9872-cc57d0da897d
function reduced_hodgkin_huxley!(du, u, p, t)
	# Voltage, K+ activation, Na+ activation, Na+ inactivation
	V, n = u 
	...
	# System of equations:
	# dV/dt I      ------- IK --------   ---------- INa ----------   ---- IL -----
    du[1] = I(t) - gK * n^4 * (V - EK) - gNa * m∞(V)^3 * (0.89 - (1.1 * n)) * (V - ENa) - gL * (V - EL)

	# dn/dt
	du[2] = (n∞(V) - n) / τn(V)
end

# ╔═╡ 3553ab02-0bd4-410b-8d60-d1b1800cbf3d
begin
	# Initial values:
	...
	# Build model
	model = ODEProblem(hodgkin_huxley!, u0, timespan, t -> 8)
end

# ╔═╡ ec006b63-b7ae-479d-9178-cb6f4d4526d5
begin
	# Initial values:
	#   Vrest        n(0) = n∞(0) 
	reduced_u0 = [0.0, 0.3176769140606974]

	# Timespan in milliseconds
	reduced_timespan = (0.0, 40.0)  

	# Build model
	reduced_model = ODEProblem(reduced_hodgkin_huxley!, reduced_u0, reduced_timespan, t -> 8)
end

# ╔═╡ d3b81f7a-b96c-4312-8e57-00e75779ffee
solution = solve(model)

# ╔═╡ eb9b449a-0cfe-4722-89bc-05fa00d4307a
reduced_solution = solve(reduced_model)

# ╔═╡ 3ee7551d-30bd-4f7e-bb6b-3b500fb4c39e
begin
	p1 = plot(
		solution, 
		idxs = (0,1),  
		color=:blue,
		title="original Hodgkin-Huxley model",
		titlefontsize=8,
		label="",
		xguide="",
		xticks=false,
	...
	p2 = plot(
		...
		color=:blue,
		linestyle=:dash,
		title="reduced Hodgkin-Huxley model",
		titlefontsize=8,
		label="",
		xguide="time (ms)",
		xguidefontsize=8,
	...
	p3 = plot(
		[reduced_solution.t, solution.t], 
		[reduced_solution[1, :], solution[1, :]], 
		color=:blue,
	...
	plot(
		...
		plot_title="Figure 5.20: Action potentials in the original (top) and\nreduced (bottom) Hodgkins-Huxley model",
		plot_titlefontsize=12,
		plot_titlevspan = 0.15
	)
end

# ╔═╡ 7380f83d-7dfa-4c0e-a9bd-d231ec7684bc
savefig("fig5_20")
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "38a4131406e922a3596a520de3a69b0adde684d0"

...
# ╔═╡ 7569975e-a364-11ef-335f-5bb4449b95d7
using Plots, DifferentialEquations

# ╔═╡ c5b25eb8-35f5-456f-afe2-b1eca95d9d14
global currents = [200,300,400,600]

# ╔═╡ 6de6ff9f-3e61-42f8-aa90-a54e298210c2
global vpeak = 25

# ╔═╡ d72a0849-b9a3-470e-814c-ff8f177c6e8b
global vr = -60

# ╔═╡ 5b423334-20c2-4676-b2e8-4163d1a42e15
global u0=[vr,0]

# ╔═╡ 85493918-b41e-459c-802d-0a5c1eb407cf
function model!(du, u, p, t)
	C,vt,k=[50,-40,1.5] # parameters used for RS
	a,b=[0.03,1]        # neocortical pyramidal neurons
	I = p
    du[1] = (I(t) + k * (u[1] - vr) * (u[1] - vt) - u[2]) / C
	du[2] = a * (b * (u[1] - vr) - u[2])
end

# ╔═╡ 740e94ba-9b44-4e6d-981c-f3ff4d396d52
function vpeak_check(u, t, integrator)
	u[1] ≥ vpeak 
end

# ╔═╡ 7f3229a6-f13d-4a60-9f6e-c10e4e2e44aa
function affect!(integrator)
	c, d = [-40,150] # neocortical pyramidal neurons
	integrator.u[1] = c
	integrator.u[2] += d
end

# ╔═╡ b9c6d3f4-02b4-4b05-a0c3-8fc6ad49f318
cb = DiscreteCallback(vpeak_check, affect!)

# ╔═╡ 42197378-fe86-4e16-9ae4-f25cbf45b462
cbs = CallbackSet(cb)

# ╔═╡ ed8a85a8-48d0-4692-ab03-a52ed713357b
begin
	global solutions = []
	
	for I=currents
		p(t)=30<t<270 ? I : 0 
		prob = ODEProblem(model!, u0, (0.0, 300.0), p)
		sol = solve(prob, Rodas5(), callback = cbs)
		push!(solutions, sol)
	end
end

# ╔═╡ a32ec2a3-fec3-4eff-9f1a-5fd3f3c9ae5c
begin
	plots = []
	
	for i=length(currents):-1:1
		push!(plots,plot(
			solutions[i].t,
			[j≥vpeak ? vpeak : j for j=solutions[i][1, :]],
			color=:mediumpurple,
			linewidth=1.2,
			label="",
			ylim=[-65,45],
	...
	push!(plots, plot(
		1:300,
		[[30<t<270 ? I : 0 for t=1:300] for I=currents],
		linewidth=1.2,
		label=reshape(["" for I=currents], 1, :),
		color=reshape([:plum for I=currents], 1, :),
		xtick=0:50:400,
		xlabel="time (ms)",
		xguidefontsize=8,
		title="Input current",
...
# ╔═╡ 6ac52017-ab57-4988-af1d-f2f9dbabd89f
begin
	layout = @layout [
	    grid(2,2)
	    a{0.15h}
	...
	plot(
		plots..., 
		layout=layout,
		plot_title="Figure 8.23: Simple model of Chattering Neuron",
		plot_titlefontsize=12
	)
end

# ╔═╡ 1a573b95-093c-4dbf-ac9d-285e08150288
savefig("../plots/fig8_23")
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "e9dd6bad1bd59ce3222b87e8d32775a5bb2e08d2"

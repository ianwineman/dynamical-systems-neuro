...
# ╔═╡ 1542e6c2-7dc7-11ef-17af-bba388f6f7d6
using Plots, DifferentialEquations, LaTeXStrings, Measures

# ╔═╡ 06585e8b-3945-4624-b9d5-32d80255a892
...
# ╔═╡ b4b8d7cf-e241-4f60-9ab3-d998a0d28134
begin
	...
	for u0=-120:10:40
		# Bistable models (I=-400)
		## Build model
		model_bi = ODEProblem(persistent_sodium!, [u0], timespan, t -> -400)
	
		## Solve model
		solution_bi = solve(model_bi, saveat=0.0:0.1:5.0)
		push!(bistable_solutions, solution_bi)

		# Bifurcation models (I=-890)
		## Build model
		model_bif = ODEProblem(persistent_sodium!, [u0], timespan, t -> -890)

		## Solve model
		solution_bif = solve(model_bif, saveat=0.0:0.1:5.0)
		push!(bifurcation_solutions, solution_bif)
	
		# Monostable models (I=-1000)
		## Build model
		model_mono = ODEProblem(persistent_sodium!, [u0], timespan, t -> -1000)
...
# ╔═╡ 7ba25607-3a43-4a68-92f4-f5a2ab50fbb7
begin
	...
	p1 = plot(
		collect(-130.0:1.0:50.0),
		[
			[(-400 - gL * (V - EL) - gNa * m∞(V) * (V - ENa)) / C 
			for V=collect(-130.0:1.0:50.0)],
			[0 for V=collect(-130.0:1.0:50.0)]
		],
		legendtitle="I=-400",
		...
		annotations = [
			...
			(-83, 15, ("rest", :left, 8)),
			(-22, 0, ("○", :left, 8)),
			(-17, -15, ("threshold", :left, 8)),
			(22, 0, ("●", :left, 6)),
			(27, 15, ("excited", :left, 8)),
			(-103, 1, ("▶", :left, 8, :blue)),
			(-73, 1, ("◀", :left, 8, :blue)),
			(-37, 1, ("◀", :left, 8, :blue)),
			(-7, 1, ("▶", :left, 8, :blue)),
	...
	p4 = plot(
		...
		ylims=(-130,50),
		annotations = [
			(5, 32, ("excited", :right, 8)),
			(5, -7, ("threshold", :right, 8)),
			(5, -78, ("rest", :right, 8)),
		],
		leftmargin=10mm
	)
	hline!([-15], linestyle=:dash, color=:black, label=:none)
	...
	plot(
		p1, p4, p2,
		p5, p3, p6,
		plot_title=L"Figure 3.33: Bifurcation in the $I_{\mathrm{Na,p}}$-model",
...
# ╔═╡ b7ad2319-c619-41ed-a268-191287850198
savefig("fig3_33.png")
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "0deebd063533d40ab528f5940b14c4f46043543d"

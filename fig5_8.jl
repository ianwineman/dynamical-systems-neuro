...
# ╔═╡ a858d99c-8d8c-11ef-0922-a377a9b113ff
using Plots, DifferentialEquations, Roots

# ╔═╡ 317e5554-ff33-40a7-a952-fcbf271d85c8
function INap_h!(du, u, p, t)
	V,h = u 
	
	gL, EL, gNa, ENa, gh, Eh = [1.3, -80, 0.9, 20, 3, -43]

	I = p
	
	# see Fast persistent² in Figure 2.20, page 46
	m∞(v) = 1 / (1 + exp((-54 - V) / 9))

	# see Ih current in Figure 2.20, page 48
	h∞(v) = 1 / (1 + exp((-75 - V) / -5.5))
	τh(v) = 1000 + 100 * exp((-1 * (-75 - v)^2) / 225)

	# System of equations:
	# dV/dt I      -- leak IL --   -- instantaneous INap --  ------ Ih -------
    du[1] = I(t) - gL * (V - EL) - gNa * m∞(V) * (V - ENa) - gh * h * (V - Eh)

	# dh/dt
	du[2] = (h∞(V) - h) / τh(V)
end

# ╔═╡ 753762c3-9845-4471-9787-281b3da26ae6
function V_nullcline(I, V)
	gL, EL, gNa, ENa, gh, Eh = [1.3, -80, 0.9, 20, 3, -43]

	# see Fast persistent² in Figure 2.20, page 46
	m∞(v) = 1 / (1 + exp((-54 - V) / 9))
	
	return (I - gL * (V - EL) - gNa * m∞(V) * (V - ENa)) / (gh * (V - Eh))
end

# ╔═╡ 0ea633fc-75d3-43da-a3f0-cc5ffa901f6b
function h_nullcline(V)
	# see Ih current in Figure 2.20
	return 1 / (1 + exp((-75 - V) / -5.5))
end

# ╔═╡ d5cf6f77-d3e6-4847-914c-c48ccd2560a5
begin
	# Initial values:
	#   Vrest                      h(0) = h∞(0)
	u0_I0 = [-51, 1 / (1 + exp((-75 + 51) / -5.5))]

	# Timespan in milliseconds
	timespan = (0.0, 1000.0)  

	# Build model
	model_I0 = ODEProblem(INap_h!, u0_I0, timespan, t -> 0)

	# Initial values:
	#   Vrest                      h(0) = h∞(0)
	u0_Ineg1 = [-59, 1 / (1 + exp((-75 + 59) / -5.5))]

	# Timespan in milliseconds
	timespan = (0.0, 1000.0)  

	# Build model
	model_Ineg1 = ODEProblem(INap_h!, u0_Ineg1, timespan, t -> -1)
end

# ╔═╡ 7d6b513a-e269-47f1-a1e9-e4f1a669c9d2
solution_I0 = solve(model_I0)

# ╔═╡ 3f127254-7c51-4f23-8599-f97fbf6025f8
solution_Ineg1 = solve(model_Ineg1)

# ╔═╡ c22e5644-f223-4cef-9bae-0467fe0df1f9
begin
	f(V) = V_nullcline(0, V) - h_nullcline(V)
	g(V) = V_nullcline(-1, V) - h_nullcline(V)
	
	interval = (-100, 40)
	f_zeros = find_zeros(f, interval...)
	g_zeros = find_zeros(g, interval...)

	# nullcline plots (I=0)
	p1 = plot(
		-100:-40,
		[V_nullcline.(0, -100:-40), h_nullcline.(-100:-40)],
		label=["V-nullcline" "h-nullcline"],
		...
		xguide="",
		xlim=[-100,-40],
		yguide="inactivation, h",
		ylim=[-0.2,1],
		yflip=true,
		grid=false,
		annotations = [
			(f_zeros[1], h_nullcline(f_zeros[1]), ("●", 8))
		]
	)
	# floating plot on top of p1 (t,V) (I=0)
	plot!(
		solution_I0, 
		idxs = (0,1),
		color=:black,
		...
		ylim=[-80,-40],
		grid=false,
		inset=(1,bbox(0.1, -0.15, 0.2, 0.33, :center, :left)),
		subplot=2
	)
	# detail inset for p1
	lens!(
		[-70,-50], [0, 0.06],
		inset=(1, bbox(0.6, 0.35, 0.25, 0.5)),
		ticks=false, 
		framestyle=:box, 
		linecolor=:green, 
		linestyle=:solid,
		bordercolor=:green,
		annotations = [
			(f_zeros[1], h_nullcline(f_zeros[1]), ("●", 6))
		],
		subplot=3
	)
	...
	p2 = plot(
		-100:-40,
		[V_nullcline.(-1, -100:-40), h_nullcline.(-100:-40)],
		label=["V-nullcline" "h-nullcline"],
		linestyle=[:solid :dash],
		color=[:red :blue],
		legendtitle="I=-1",
		...
		grid=false,
		annotations = [
			(g_zeros[1], h_nullcline(g_zeros[1]), ("○", 8))
		]
	)
	# floating plot on top of p2 (t,V) (I=-1)
	plot!(
		solution_Ineg1, 
		idxs = (0,1),
		color=:black,
	...
	plot(
		p1, p2,
		layout=grid(2,1),
		plot_title="Figure 5.8: Rest and sustained subthreshold oscillations\nin the \$I_{Na,p}+I_h\$-model",
		size=(800,600),
		plot_titlevspan=0.1
	)
end

# ╔═╡ ce6ef453-1b67-4d7c-ad09-c4e47983302e
savefig("fig5_8")
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "8091dcc1c485b65f9135dc2b37c499dcf4de6d08"

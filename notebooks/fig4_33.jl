...
# ╔═╡ 9fd2a1d8-da7a-46f9-b009-9d92af9bf496
function m∞(V)
	Vhalf, k = -20, 15
	return 1 / (1 + exp((Vhalf - V) / k))
end

# ╔═╡ 140d9296-f676-475b-85df-c4dd94fc7376
function n∞(V)
	Vhalf, k = -45, 5
	return 1 / (1 + exp((Vhalf - V) / k))
end

# ╔═╡ 9090b763-588a-41ec-9f64-6c450c52951f
...
# ╔═╡ 456f9d73-251b-4cfd-bdb2-b7c576be3118
begin
	u0 = [-65.0, n∞(-65.0)]
	timespan = (0.0, 120.0)  
	model = ODEProblem(INapIK_model!, u0, timespan, t -> 3t/10)
end

# ╔═╡ 8bfcf255-9a19-47e3-b310-27fb489fda03
solution = solve(model)

# ╔═╡ c4aa82d6-715f-495e-8f7c-d5bc60daf773
begin
	p1 = plot(
		solution, 
		idxs = (0,1),
	...
	p2 = plot(
		[i for i=0.0:0.1:120.0],
		[(3*i)/10 for i=0.0:0.1:120.0],
		linewidth=2,
		color=:crimson,
		xlims=[0,120],
		ylims=[0,36],
		xguide="time (ms)",
		yguide="injected current, I",
		xguidefontsize=8,
		yguidefontsize=8,
		label="",
	)
...
# ╔═╡ b44b9bac-6dcd-4a63-b55d-b3e5cf9d595e
savefig("fig4_33")
